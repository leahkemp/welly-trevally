# Population stucture and demographic history of *P. georgianus* {#popStructure}
...
# Include putative substock Northland and rest of NZ?
...
```{r putativeSubStocks, out.width = "70%", fig.align = "center", fig.cap = "Putative sub-stocks of $\\textit{P. georgianus}$ in New Zealand.", fig.show = "hold"}
library(knitr)
...
```{r primersControl, out.width = "100%", fig.align = "center", fig.cap = "The alignment positions of two $\\textit{P. georgianus}$ alignments showing the level of within species variation based on thirteen broodstock $\\textit{P. georgianus}$ (blue) and 304 $\\textit{P. georgianus}$ sampled in New Zealand (black)."}
library("knitr")
...
```{r differentStatAreas, out.width = "100%", fig.align = "center", fig.cap = "Statistical areas from which $\\textit{P. georgianus}$ are genetically distinct (FST's are provided)."}
library(knitr)
...
# Import haplotype data of those similar to juveniles sampled from Whangarei
...
```{r similarHaplotypes, out.width = "60%", fig.align = "center", fig.cap = "Sampling locations of adult $\\textit{P. georgianus}$ with the same haplotypes as juveniles caught in Whangarei (top) or haplotypes with one (middle) or two (bottom) single nucleotide differences.", fig.show = "hold"}
library("knitr")
...
A haplotype network of *P. georgianus* from the Bay of Plenty and TRE2 show these fish are highly genetically connected (see Figure \@ref(fig:haploTRE2andBOP)) and an AMOVA did not find evidence to suggest they are distinct stocks (see Table \@ref(tab:amovaTRE2andBOP)).

Within TRE7, the haplotype lineages among the three putative sub-stocks did not appear to be distinct (see Figure \@ref(fig:haploSubstocksTRE7)). However, an AMOVA analysis found evidence to suggest that at least one pair of putative sub-stocks were significantly genetically distinct at the 10% significance level ($F_{ST}$: 0.01004, $p$-value: 0.059) (see Table \@ref(tab:amovaSubstocksTRE7)). Pairwise comparisons identified that the first putative sub-stock (Ninety Mile Beach) was distinct from the third putative sub-stock (South Taranaki Bight) ($F_{ST}$: 0.03019, $p$-value: 0.039) (see Table: \@ref(tab:amovaAmongSubstocksTRE7)).
...
There was some subtle grouping of haplotypes of *P. georgianus* in New Zealand, however the groupings were not associated with QMA's (see Figure \@ref(fig:haploQMAs)). Instead, the haplotypes sampled from each of these three QMA's were highly genetically linked. An AMOVA did not find evidence to suggest *P. georgianus* haplotypes from TRE1, TRE2 and TRE7 are genetically distinct ($p$-value: 0.498, see Table \@ref(tab:amovaQMAs)). 
...
```{r haploTRE2andBOP, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of $\\textit{P. georgianus}$ sampled from the Bay of Plenty and TRE2."}
library("knitr")
...
read.csv("../data/08-popStructure/AMOVAs/TRE2andBOP.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               caption = "AMOVA output comparing $\\textit{P. georgianus}$ from TRE2 and Bay of Plenty.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
...
```{r haploSubstocksTRE7, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of $\\textit{P. georgianus}$ from three putative substocks within TRE7."}
library("knitr")
knitr::include_graphics("../imagesFinal/08-popStructure/haploSubstocksTRE7.png")
```

```{r amovaSubstocksTRE7}
library(dplyr)
library(kableExtra)
library(float)
...
read.csv("../data/08-popStructure/AMOVAs/SubstocksTRE7.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               caption = "AMOVA output comparing $\\textit{P. georgianus}$ from three putative substocks within TRE7.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
...
read.csv("../data/08-popStructure/AMOVAs/amongSubstocksTRE7.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               caption = "AMOVA output of three pairwise comparisons of three putative substocks of $\\textit{P. georgianus}$ within TRE7.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
  kable_styling(latex_options = "scale_down") %>%
  pack_rows("Putative substock 1 and 2", 1, 3) %>%
  pack_rows("Putative substock 2 and 3", 4, 6) %>%
  pack_rows("Putative substock 1 and 3", 7, 9)
```
...
```{r haploIslands, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of $\\textit{P. georgianus}$ from Three Kings Islands and Kermadec Islands."}
library("knitr")
...
read.csv("../data/08-popStructure/AMOVAs/QMAs.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               caption = "AMOVA output comparing Trevally from three Quota Management Areas (QMA's) in New Zealand.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
...
```{r haploJuveniles, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of juvenile $\\textit{P. georgianus}$ from Whangarei (New Zealand), adult $\\textit{P. georgianus}$ from Western Australia and adult $\\textit{P. georgianus}$ from New Zealand."}
library("knitr")
...
read.csv("../data/08-popStructure/AMOVAs/NZAUSBabies.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               ...
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "scale_down") %>%
  kable_styling(latex_options = "scale_down") %>%
...
```{r haploAus, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of $\\textit{P. georgianus}$ from three locations in Western Australia."}
library("knitr")
...
read.csv("../data/08-popStructure/AMOVAs/AUS.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation", "FST", "Significance"),
               caption = "AMOVA output comparing $\\textit{P. georgianus}$ from three regions in Western Australia.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
...
```{r haploNZandAUS, out.width = "100%", fig.align = "center", fig.cap = "Haplotype network and sampling locations of $\\textit{P. georgianus}$ from New Zealand and Western Australia."}
...
read.csv("../data/08-popStructure/AMOVAs/AUSandNZ.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  knitr::kable(col.names = c("", "Degrees of freedom", "Sum of squares", "Sigma squared", "Variation (%)", "FST", "Significance"),
               caption = "AMOVA output comparing $\\textit{P. georgianus}$ from New Zealand and Western Australia.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
...
When the support for the free migration model (AUS $\Leftrightarrow$ NZ) was compared to the NZ $\Rightarrow$ AUS (85.51668) and AUS $\Rightarrow$ NZ models (79.48104), a positive log Bayes factor was returned indicating the most highly supported model was the AUS $\Leftrightarrow$ NZ model. For MCMC sampling, the acceptance ratios of this model was roughly half (see Table \@ref(tab:acceptanceRatios)) and there was a large sample size for all model parameters (see Table \@ref(tab:autocorrelation)). The autocorrelation between MCMC sampled updates for theta1 and theta2 was minimal (see Table \@ref(tab:autocorrelation). However, there was some autocorrelation between the sampled updates of both migration rates (0.73492 and 0.80200) that was not fully resolved by lowering the update frequency of the MCMC sampling. The distributions of the sampled posterior probabilities of all four estimated parameters (population size of New Zealand and Western Australia over time (theta 1 and 2) and migration each direction over time (migration AUS to NZ and migration NZ to AUS)) can be found in the Appendix (see Figure \@ref(fig:posteriorDistribution)).
...
read.csv("../data/08-popStructure/haplotypesSummary.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  ...
  knitr::kable(col.names = c("Population", 
                             "Sample size", 
                             "Nucleotide diveristy", 
                             "Haplotype diversity", 
                             "Polymorphic sites",
                             "Average number of differences between haplotypes", 
                             "Transitions", 
                             "Transversions", 
                             "Substitutions", 
                             "Insertions and Deletions (indels)"), 
               caption = "Nucleotide and haplotype diversity of New Zealand and Western Australian populations of $\\textit{P. georgianus}$ based on partial control region sequences.",
               booktabs = TRUE,
               align = "r") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
  kable_styling(latex_options = "scale_down") %>%
  add_header_above(c(" " = 2, "Overall diversity" = 4, "Types of genetic diversity (by percentage of sites)" = 4)) %>%
  column_spec(column = 3:6, width = "0.6in") %>%
  column_spec(column = 10:10, width = "0.6in")
```
...
```{r demoClass, out.width = "100%", fig.align = "center", fig.cap = "Demographic classes of New Zealand and Western Australian $\\textit{P. georgianus}$ compared to several other New Zealand fishery species."}
library("knitr")
...
```{r mismatch, out.width = "100%", fig.align = "center", fig.cap = "Mismatch distribution for both populations of $\\textit{P. georgianus}$ including confidence intervals."}
library(knitr) 
...
read.csv("../data/08-popStructure/acceptanceRatios.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(
    col.names = c("Parameter", "Accepted changes", "Ratio"),
    caption = "Acceptance ratios for all parameters of the final migration model run.",
    booktabs = TRUE,
    align = "r",
    linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
...
read.csv("../data/08-popStructure/autocorrelationAndSamplesize.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(
    col.names = c("Parameter", "Autocorrelation", "Effective sample size"),
    caption = "MCMC autocorrelation and effective sample sizes for all parameters of the final migration model run.",
    booktabs = TRUE,
    align = "r",
    linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
...
read.csv("../data/08-popStructure/controlSequencesSummary.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("Country", "QMA", "Statistical area", "Sample source", "Tissue type", "Sampling date", "Exact GPS known", "Number of samples"),
               booktabs = TRUE,
               caption = "Information on all $\\textit{P. georgianus}$ samples for which the control region was sequenced.",
               align = "r",
               linesep = "") %>%
  kableExtra::kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
  kable_styling(latex_options = "scale_down") %>%
  add_header_above(c("Sampling location" = 3, " " = 5))
```
...
```{r skyline, out.width = "100%", fig.align = "center", fig.cap = "Skyline plots showing modelled changes in population sizes and migration rates over time of New Zealand and Western Australian $\\textit{P. georgianus}$ populations including error bars (1.96 x approximate standard error)."}
library(knitr) 
