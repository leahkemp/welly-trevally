# Evolutionary relationships of *Pseudocaranx* species {#evolutionaryRelationships}
...
The editing of the DNA sequences and alignment was carried out in Geneious version 11.1.5 [@Kearse2012] The *Pseudocaranx* sequences and one outgroup (*Seriola lalandi*, GenBank accession: KM877619) were aligned using the Geneious alignment tool with the default settings (Alignment type: global alignment with free end gaps, Cost matrix: 65% similarity, Gap open penalty: 12, Gap extension penalty: 3, Refinement iterations: 2). The alignments were manually edited to removed base pair calling noise and areas were the primer had been incorporated into the sequence. The chromatogram files of the DNA sequences were visually checked for signal strength and unambiguous base calling. The final alignment covered a 602 base pair region of the COI gene, excluding the first 96 and last 853 base-pairs of the gene (see Figure \@ref(fig:primersCoi) and see Figure \@ref(fig:alignmentCoi1) in the Appendix for the final COI gene alignment).
...
```{r primersCoi, out.width = "100%", fig.align = "center", fig.cap = "COI primer binding locations and within species variation among thirty $\\textit{P. georgianus}$ individuals from New Zealand (black) and thirteen broodstock $\\textit{P. georgianus}$ (blue)."}
library("knitr")
...
In total, partial COI sequences for eight *P. dentex*, five *P. dinjerra*, thirty *P. georgianus* and sixteen *P. wrighti* individuals were either procured or sequenced (see Table \@ref(tab:phylogenyLabels)).
...
See Figure \@ref(fig:cladeLocations) and Table \@ref(tab:phylogenyLabels) for the sampling locations of *Pseudocaranx* species from these four phylogenetic clades.
...
```{r phylogenyCoiMaxLikelihood, out.width = "100%", fig.align = "center", fig.cap = "Maximum likelihood $\\textit{Pseudocaranx}$ phylogeny including bootstrap support greater than 50."}
library(phylocanvas)
knitr::include_graphics("../imagesFinal/07-evolutionaryRelationships/phylogenyCoiMaxLikelihood.png")
```

```{r phylogenyCoiBayes, out.width = "100%", fig.align = "center", fig.cap = "Bayesian $\\textit{Pseudocaranx}$ phylogeny including posterior probabilities greater than 0.5."}
...
```{r cladeLocations, out.width = "100%", fig.align = "center", fig.cap = "Sampling locations of the four major $\\textit{Pseudocaranx}$ phylogenetic clades including $\\textit{P. georgianus}$ (red), $\\textit{P. dentex}$ (purple), $\\textit{P. dinjerra}$ (yellow) and $\\textit{P. wrighti}$ (green)."}
library("knitr")
...
read.csv("../data/06-fieldAndLab/dnaSequences.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  format_cells(1:59, 1, "italics") %>%
  knitr::kable(col.names = c("Label", "Region", "Area", "Source", "GenBank accession", "BOLD ID", "Museum ID"),
               caption = "Information on all COI sequences that were acquired or sequenced for $\\textit{Pseudocaranx}$ species.",
               booktabs = TRUE,
               align = "r",
               linesep = "",
               escape = FALSE) %>%
  ...
  add_header_above(c(" " = 1, "Sampling location" = 2, " " = 4))
```
