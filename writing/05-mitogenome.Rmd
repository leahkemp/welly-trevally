# Assembly and description of the *P. georgianus* mitogenome {#mitogenome}
...
BLAST searches and a literature search were used to establish species that are closely related to *P. georgianus* for which complete annotated mitogenome is available. The COI gene of a *P. georgianus* individual sampled from Victoria, Australia (GenBank accession: EF609442) was blasted against a nucleotide collection database using a discontiguous megablast that handles more dissimilar sequences like inter-species variation [@Biomatters2018]. This BLAST search queried the National Center for Biotechnology Information (NCBI) within Geneious version 11.1.5 [@Kearse2012] and returned a list of genetically similar sequences and their ranked pairwise identity scores.
...
bind_rows(contigDataLongKX, contigDataLongKM, contigDataLongAP) %>%
...
```{r unresolvedRegions, out.width = "100%", fig.align = "center", fig.cap = "Uncertainty in the consensus mitogenome resulting from assembling whole genome data of one $\\textit{P. georgianus}$ individual ($\\textit{Broodstock trevally 1}$) to three reference mitogenomes."}
library(knitr)
...
image <- read.csv("../data/05-mitogenome/summaryDiffIterationsContigs.csv", header = TRUE) %>%
  ...
  scale_color_discrete(name = "Individual",
                       ...
                       labels = c("$\\textit{P. georgianus}$ 1", "$\\textit{P. georgianus}$ 2", "$\\textit{P. georgianus}$ 3"))
                       
ggsave(file = "../images/05-mitogenome/iterationsIdenticalSites.svg", width = 9, height = 3, device = "svg", units = "in")
```

```{r iterationsIdenticalSites, out.width = "100%", fig.align = "center", fig.cap = "Proportion of identical sites of the mitogenome assemblies of three $\\textit{P. georgianus}$ individuals to three reference genomes related to the number of mapping iterations."}
library(knitr)
...
```{r coverageDiffIterations, out.width = "100%", fig.align = "center", fig.cap = "The contig depths and percentage identities of whole genome data of one individual ($\\textit{Broodstock trevally 1}$) mapped to the $\\textit{C. equula 2}$ reference mitogenome using different numbers of mapping iterations."}
library(knitr)
...
```{r identityDiffIterations, out.width = "100%", fig.align = "center", fig.cap = "Unresolved regions along the mitogenome of one individual ($\\textit{Broodstock trevally 1}$) mapped to $\\textit{C. equula 2}$ using different numbers of mapping iterations."}
library(knitr)
...
```{r coverageDiffIndividuals, out.width = "100%", fig.align = "center", fig.cap = "A comparison of contig scaffolds from thirteen $\\textit{P. georgianus}$ individuals mapped to $\\textit{C. equula 2}$ including the range in contig depth and percentage identity at each nucleotide position."}
library(knitr)
...
```{r identicalAndIdentity, out.width = "100%", fig.align = "center", fig.cap = "The association between the average number of identical sites and the average pairwise identity along the length of the mitogenome with contig depth of whole genome data of thirteen $\\textit{P. georgianus}$ individuals assembled to $\\textit{C. equula 2}$."}
library(knitr) 
...
read.csv("../data/05-mitogenome/intergenicRegions.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  knitr::kable(col.names = c("Length", "Bordering regions"),
               caption = "Intergenic regions in the $\\textit{P. georgianus}$ mitogenome.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
...
read.csv("../data/05-mitogenome/mtgenomedata.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  knitr::kable(col.names = c("Gene", "Start position", "Stop position", "Length", "Direction", "Strand encoded", "Overlapping with neighbour", "Start codon", "Stop codon", "GC content"), 
               caption = "Description of the genes in the $\\textit{P. georgianus}$ mitogenome.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
  kable_styling(latex_options = "scale_down") %>%
  column_spec(column = 2:3, width = "0.6in") %>%
  column_spec(column = 6:7, width = "0.6in")
```

#### tRNA's and rRNA's {-}

The *P. georgianus* mitogenome also encodes two rRNA's on the light strand and twenty-two tRNA's in the light and heavy strands, including two duplicate tRNA's (Ser, Leu) (see Table \@ref(tab:trnaRrnaInfo)). Three pairs of tRNA's overlap (Ile with Gln, Gln with Met and Thr with Pro). The GC content of the tRNA's and rRNA's range from 30.4% for tRNA-Arg to 50.7% for tRNA-Ile. The length of the tRNA's are roughly similar (68-75 base-pairs).
...
d %>% 
  ...
  knitr::kable(col.names = c("", "Start position", "Stop position", "Length", "Direction", "Strand encoded", "Overlapping with neighbour", "GC content"), 
               caption = "Description of the tRNA's and rRNA's in the $\\textit{P. georgianus}$ mitogenome.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position") %>%
  pack_rows("rRNA's", 1, 2) %>%
  pack_rows("tRNA's", 3, 24) %>%
  column_spec(column = 6:7, width = "0.6in")
```
...
read.csv("../data/05-mitogenome/mtgenomedata.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  knitr::kable(col.names = c("DNA region", "Length", "Number", "Proportion of region", "Number", "Proportion of region"), 
               caption = "Within species variation along the  $\\textit{P. georgianus}$ mitogenome.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(position = "center", font_size = 8, latex_options = "hold_position") %>%
  add_header_above(c(" " = 2, "Single nucleotide polymorphisms" = 2, "Parsimony informative sites" = 2))
```
...
read.csv("../data/05-mitogenome/conservedRegions.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  knitr::kable(col.names = c("DNA region", "Start", "End", "Length", "Conservation value", "Significance"), 
               caption = "Conserved regions in the $\\textit{P. georgianus}$ mitogenome.",
               booktabs = TRUE,
               align = "r",
               linesep = "",
               escape = FALSE) %>%
