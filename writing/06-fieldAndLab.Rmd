# Field and laboratory work {#fieldAndLab}
...
leaflet(data = popgen_data) %>%
  ...
  mapshot(file = "../images/06-fieldAndLab/sampling.png", vwidth = 6300, vheight = 4100)  
```

```{r sampling, out.width = "100%", fig.align = "center", fig.cap = "All sampling locations of $\\textit{P. georgianus}$."}
library("knitr")
knitr::include_graphics("../imagesFinal/06-fieldAndLab/sampling.png")
```
...
The COI gene of fourteen *P. georgianus* fish sampled from New Zealand's North Island were PCR amplified and sequenced using the final PCR protocol below. The final components of the PCR reaction are described in Table \@ref(tab:coiPCR)) and summary information on all final *P. georgianus* COI sequences can be found in Table \@ref(tab:summaryCOI) (see Chapter \@ref(evolutionaryRelationships) for more detailed information on the COI sequences).
...
read.csv("../data/06-fieldAndLab/primersCoi.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  ...
  knitr::kable(col.names = c("Primer name", "Direction", "Primer sequence (5' to 3')", "GC content"),
               caption = "A description of the primers designed to target the COI gene of $\\textit{P. georgianus}$.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
...
read.csv("../data/06-fieldAndLab/coiPCR.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("PCR component", "Concentration", "Volume (μl)"),
               caption = "Components of the PCR reaction used to amplify the partial COI gene of $\\textit{P. georgianus}$.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
...
The control region of 389 *P. georgianus* fish sampled from New Zealand's North Island and Western Australia were PCR amplified and sequenced using the final PCR protocol below. The final components of the PCR reaction are described in Table \@ref(tab:controlPCR)) and summary information on all final *P. georgianus* control region sequences can be found in Table \@ref(tab:summaryControl) (see Chapter \@ref(popStructure) for more detailed information on the control region sequences).
...
read.csv("../data/06-fieldAndLab/primersControl.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  dplyr::select("name", "direction", "sequence", "GCcontent") %>%
  knitr::kable(col.names = c("Primer name", "Direction", "Primer sequence (5' to 3')", "GC content"),
               caption = "A description of the primers designed to target the control region of $\\textit{P. georgianus}$.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
...
read.csv("../data/06-fieldAndLab/controlPCR.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("PCR component", "Concentration", "Volume (ul)"),
               caption = "Components of the PCR reaction used to amplify the partial control region of $\\textit{P. georgianus}$.",
               booktabs = TRUE,
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
...
read.csv("../data/06-fieldAndLab/dnaSequences.csv", stringsAsFactors = FALSE, header = TRUE, na.strings = c("")) %>%
  ...
  dplyr::summarise(n()) %>%
  format_cells(1:18, 1, "italics") %>%
  knitr::kable(col.names = c("Species", "Sampling location","Source", "Number of sequences"),
               caption = "Summary information on all COI sequences that were acquired and sequenced for $\\textit{Pseudocaranx}$ species.",
               booktabs = TRUE,
               align = "r",
               linesep = "",
               escape = FALSE) %>%
...
read.csv("../data/06-fieldAndLab/summaryControl.csv", stringsAsFactors = FALSE, header = TRUE, na.strings=c("")) %>%
  knitr::kable(col.names = c("Sampling country", "Sampling QMA", "Sample source", "Number of sequences"),
               booktabs = TRUE,
               caption = "Summary information on all $\\textit{P. georgianus}$ samples for which the control region was sequenced.",
               align = "r",
               linesep = "") %>%
  kable_styling(font_size = 8, position = "center", latex_options = "hold_position")
```